  ...
  return (
    <div className="page-scroll space-y-10 lg:space-y-14">
      <section
        ...
        className="reveal relative overflow-hidden rounded-[2rem] border border-white/20 bg-white/10 px-6 py-16 sm:px-10 lg:min-h-[calc(100vh-9rem)] lg:px-14 lg:py-24"
      >
        ...
        <div className="absolute bottom-[18%] right-[18%] hidden h-10 w-10 rounded-full border border-white/50 bg-white/28 backdrop-blur-sm lg:block" />
        ...
        <div className="relative mx-auto flex min-h-[calc(100vh-14rem)] max-w-4xl flex-col items-center justify-center text-center">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {['Fresh produce', 'Fast checkout', 'Trusted supply', 'Single page flow'].map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/45 bg-white/20 px-4 py-2 text-sm text-slate-600 backdrop-blur-sm"
              >
                {item}
              </span>
            ))}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            {['Home', 'About', 'Services', 'Contact'].map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/35 bg-white/20 px-4 py-2 text-sm text-slate-600 backdrop-blur-sm"
              >
                {item}
              </span>
            ))}
        ...
        <div className="space-y-4">
          {highlights.map((item) => {
            ...
            return (
              <article
                key={item.title}
                className="rounded-[1.8rem] border border-white/20 bg-white/10 p-6 backdrop-blur-sm"
              >
      ...
      <section
        ...
        className="reveal grid gap-6 lg:min-h-[78vh] lg:grid-cols-[0.95fr_1.05fr] lg:items-start"
      >
        <div className="space-y-4">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {services.map((service) => (
              <div
                key={service}
                className="rounded-2xl border border-white/30 bg-white/14 px-4 py-4 text-sm font-medium text-slate-600 backdrop-blur-sm"
              >
                {service}
              </div>
            ))}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-[1.8rem] border border-white/20 bg-blue-50/35 p-6 backdrop-blur-sm sm:col-span-2">
            ...
            <p className="mt-3 text-slate-600">
              The content stays separate and scrollable so each section can breathe on a laptop display.
            </p>
          </div>
        </div>
      </section>
      ...
      <section
        ...
        className="reveal grid gap-6 lg:min-h-[72vh] lg:grid-cols-[0.96fr_1.04fr] lg:items-start"
      >
        <div className="space-y-4">
          <span className="section-kicker">Contact</span>
          <h2 className="max-w-xl text-3xl font-bold text-primary-strong sm:text-4xl">
            A minimal contact section to close the page.
          </h2>
          <p className="text-slate-600">
        ...
        <div className="rounded-[1.8rem] border border-white/20 bg-white/10 p-6 backdrop-blur-sm sm:p-8">
